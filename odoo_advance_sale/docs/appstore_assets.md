...
The migrated banner was edited using the built-in image generation tool: replace
only the small ODOO 19 label with DELIVERY FIRST, preserve the existing text,
illustration, palette, safe margins and 2:1 aspect ratio. The same neutral
banner is used for versions 17 and 18; the HTML and manifest identify the version.
